...
function Product(id, name, desc, price, discont, discvalue, tags){    //Конструктор продукта
 ...
 function renderBasket(basket){
     ...
     if (basket.products.length == 0){
         totalEl.textContent = `Корзина пуста`
         basketEl.appendChild(totalEl);
     } else {
         totalEl.textContent = `В корзине ${basket.products.length} товаров на сумму ${basket.totalprice} рублей.`
         basketEl.appendChild(totalEl);
     }
 }
 ...
 button1.addEventListener('click', () => {
     addprod(basket1, product1);
     renderBasket(basket1);
 });

 const button2 = document.querySelector('#add2');
 button2.addEventListener('click', () => {
     addprod(basket1, product2);
     renderBasket(basket1);
 });
 ...
 const button4 = document.querySelector('#del1');
 button4.addEventListener('click', () => {
     delprod(basket1, product1);
     renderBasket(basket1);
 });

 const button5 = document.querySelector('#del2');
 button5.addEventListener('click', () => {
     delprod(basket1, product2);
     renderBasket(basket1);
 });

 const button6 = document.querySelector('#del3');
 button6.addEventListener('click', () => {
     delprod(basket1, product3);
     renderBasket(basket1);
 });